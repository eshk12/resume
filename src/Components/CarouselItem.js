    ...
    return (
        <Card className={"carouselItem"}>
            ...
            <div className={"carouselItem-content"}>
                ...
                <div className="links">
                    {
                        carouselItem.links.map((link, key) => <React.Fragment key={key}><Button className="me-1" ><a href={link.url} target="_blank" rel="noopener noreferrer">{link.linkName}</a></Button> {link.accessDetails && <span className="accessDetails">{link.accessDetails}</span>}</React.Fragment> )
                    }
                </div>
            </div>
        </Card>
